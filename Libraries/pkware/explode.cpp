...
static unsigned char DistCode[] = {
    ...
    0x42, 0x02, 0x7C, 0x3C, 0x5C, 0x1C, 0x6C, 0x2C, 0x4C, 0x0C, 0x74,
    0x34, 0x54, 0x14, 0x64, 0x24, 0x44, 0x04, 0x78, 0x38, 0x58, 0x18,
    0x68, 0x28, 0x48, 0x08, 0xF0, 0x70, 0xB0, 0x30, 0xD0, 0x50, 0x90,
    0x10, 0xE0, 0x60, 0xA0, 0x20, 0xC0, 0x40, 0x80, 0x00};

static unsigned char ExLenBits[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static unsigned char LenBits[] = {0x03, 0x02, 0x03, 0x03, 0x04, 0x04,
                                  0x04, 0x05, 0x05, 0x05, 0x05, 0x06,
                                  0x06, 0x06, 0x07, 0x07};

static unsigned char LenCode[] = {0x05, 0x03, 0x01, 0x06, 0x0A, 0x02,
...
static void GenDecodeTabs(
    ...
    unsigned char *length_bits, // [in] Table of lengths. Each length is stored
                                // as number of bits
...
static int WasteBits(TDcmpStruct *pWork, unsigned int nBits) {
  ...
  if (pWork->in_pos == pWork->in_bytes) {
    ...
    if ((pWork->in_bytes = pWork->read_buf((char *)pWork->in_buff,
                                           &pWork->in_pos, pWork->param)) == 0)
...
static unsigned int DecodeLit(TDcmpStruct *pWork) {
  ...
  // Test the current bit in byte buffer. If is not set, simply return the next
  // 8 bits.
  if (pWork->bit_buff & 1) {
...
static unsigned int DecodeDist(TDcmpStruct *pWork, unsigned int rep_length) {
  ...
  if (rep_length == 2) {
    ...
    distance = (dist_pos_code << pWork->dsize_bits) |
...
static unsigned int Expand(TDcmpStruct *pWork) {
  ...
  // The returned literal can either be an uncompressed byte (next_literal <
  // 0x100) or an encoded length of the repeating byte sequence that is to be
  // copied to the current buffer position
  while ((result = next_literal = DecodeLit(pWork)) < 0x305) {
    ...
    if (next_literal >= 0x100) {
      ...
      unsigned int minus_dist; // Backward distance to the repetition, relative
    ...
    if (pWork->outputPos >= 0x2000) {
      ...
      // This is needed because the decompression might reuse them as
      // repetitions. Note that if the output buffer overflowed previously, the
      // extra decompressed bytes are stored in "out_buff_overflow", and they
      // will now be within decompressed part of the output buffer.
      memmove(pWork->out_buff, &pWork->out_buff[0x1000],
...
unsigned int
explode(unsigned int (*read_buf)(char *buf, unsigned int *size, void *param),
  ...
  pWork->in_pos = sizeof(pWork->in_buff);
  pWork->in_bytes =
      pWork->read_buf((char *)pWork->in_buff, &pWork->in_pos, pWork->param);
  if (pWork->in_bytes <= 4)
    return CMP_BAD_DATA;

  pWork->ctype =
      pWork->in_buff[0]; // Get the compression type (CMP_BINARY or CMP_ASCII)
  ...
  pWork->dsize_mask =
      0xFFFF >> (0x10 - pWork->dsize_bits); // Shifted by 'sar' instruction
